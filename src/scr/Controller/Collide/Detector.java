        ...
        switch (s1.shape)
        {
            case Point:
                switch (s2.shape)
                {
                    ...
                    case Rect:
                        return isPointWithRect(s1.x, s1.y, s2.x, s2.y, s2.w, s2.h);
                    case Oval:
                        return isPointWithOval(s1.x, s1.y, s2.x, s2.y, s2.r);
                }
            break;
            case Oval:
                switch (s2.shape)
                {
                    case Point:
                        return isPointWithOval(s2.x, s2.y, s1.x, s1.y, s1.r);
                    case Oval:
                        return isCollisionWithCircle(s1.x, s1.y, s2.x, s2.y, s1.r, s2.r);
                    case Rect:
                        return isArcRectCollides(s1.x, s1.y, s1.r, s2.x, s2.y, s2.w, s2.h);
                }
            break;
            case Rect:
                switch (s2.shape)
                {
                    case Rect:
                        return isCollisionWithRect(s1.x, s1.y, s1.w, s1.h, s2.x, s2.y, s2.w, s2.h);
                    case Oval:
                        return isArcRectCollides(s2.x, s2.y, s2.r, s1.x, s1.y, s1.w, s1.h);
                    case Point:
                        return isPointWithRect(s2.x, s2.y, s1.x, s1.y, s1.w, s1.h);
                }
            break;
        }
        return false;
