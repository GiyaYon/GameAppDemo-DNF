...
public class GiselleAnimator extends CharacterAnimator
{
    public GiselleAnimator(CharacterBaseModel cb) {
        this.cb = cb;
    ...
    protected void setAnimResources() throws IOException {
        animResources.res.put("body",new ResSprites("giselle\\body"));
    }
    ...
    protected void setAnimations() {
        ...
        //先装配
        assembleToAnimations("idle",0,2,true);
        assembleToAnimations("walk",2,9,true);
        assembleToAnimations("attack1",10,18,false);
        assembleToAnimations("attack2",15,21,false);
        assembleToAnimations("injure",30,31,false);
        assembleToAnimations("inair",32,33,false);
        assembleToAnimations("death",35,36,false);
    }
    ...
    protected void setStates() {
        ...
        statesList.put(GiselleStatesTable.Attack,new Attack(cb));
        statesList.put(GiselleStatesTable.Attack2,new Attack2(cb));
    ...
    public void render(Graphics g, JPanel jPanel, Transform transform) {
        ...
        if(cb.property.states.equals(GiselleStatesTable.Jump) || cb.property.states.equals(GiselleStatesTable.Fall) || cb.property.states.equals(BaseStates.InAir) || cb.property.states.equals(BaseStates.Throw))
        {
            Transform transform1 = new Transform(transform.xPos, cb.property.horizontal.y);
            transform1.flipX = transform.flipX;
            animator.Flash(g,jPanel,transform1,700,700);

            //animator.Flash(g,jPanel, transform);
        }
        else
        {
            animator.Flash(g,jPanel, transform,700,700);
        }
